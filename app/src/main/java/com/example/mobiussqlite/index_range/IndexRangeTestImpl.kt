...
class IndexRangeTestImpl(context: Context) {

    private val openHelper by lazy { OpenHelper(context, "index_range_test_impl") }
    ...
    private fun queryData(count: Int): Long {
        ...
        var durationNs = 0L
        val query = "SELECT id FROM messages WHERE time < ? ORDER BY time DESC LIMIT 50"
        val args = arrayOf("")
        db.beginTransactionNonExclusive()
